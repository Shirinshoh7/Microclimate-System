"""
In-memory data storage and state management.

Manages active climate profile, current sensor data, historical data queue,
...
class DataStorage:
    """Global data storage and state management.
    
    Attributes:
        current_data (dict): Latest sensor readings (temperature, humidity, CO2, CO)
        data_history (deque): Historical data points (FIFO queue)
        active_websockets (list): Connected WebSocket clients
        active_profile (dict): Currently active climate profile with thresholds
    """
    ...
    def _load_active_profile(self) -> Dict:
        """Load active climate profile from persistent storage file.
        
        Returns:
            dict: Active profile with thresholds, or default profile if not found
        """
        try:
            ...
            print(f"⚠️ Failed to load active_profile.json: {e}")
        return dict(PROFILES[0])

    def _save_active_profile(self) -> None:
        """Persist active climate profile to storage file.
        ...
        try:
            ...
            print(f"⚠️ Failed to save active_profile.json: {e}")
    ...
    def _evaluate_norm(
        ...
        """Check if current readings are within active profile thresholds.
        
        Returns:
            dict: Status object with is_danger flag and list of violated parameters
        """
    ...
    def update_current_data(self, data: Dict):
        ...
        norm = self._evaluate_norm(temperature, humidity, co2_ppm, co_ppm)
